...
import type {
  ...
  ModuleSlot, InstallModuleRequest, CurrentStats,
} from '@/types'
...
export const shipsApi = {
  ...
  modules: {
    list:    (shipId: string)                               => api.get<ModuleSlot[]>(`/ships/${shipId}/modules`),
    ...
    remove:  (shipId: string, slot: number)                 => api.delete<void>(`/ships/${shipId}/modules/${slot}`),
  },
